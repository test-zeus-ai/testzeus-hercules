...
def generate_features_report(virtuoso_data) -> str:
    """
    Generate a human-readable test steps report where:
      - 'journeys' are called 'Features'
      - 'cases'   are called 'Scenarios'
    
    Returns a single string that can be written to a .txt file or printed.
    """
            ...
            for step_idx, step in enumerate(steps, start=1):
                ...
                for sel in selectors:
                    if sel.get("type") == "GUESS":
                        ...
                        # If it looks like JSON (e.g. {"clue":"some text"}), parse it
                        clue_text = raw_val
...
if __name__ == "__main__":
    """
    Usage example:
      python script.py raw_input.json output_report.txt
    """
    ...
    if len(sys.argv) > 1:
        ...
        input_file = "raw_jsons/test1.json"
    
    if len(sys.argv) > 2:
        output_file = sys.argv[2]
    else:
        output_file = "output_txt/test_report.txt"
