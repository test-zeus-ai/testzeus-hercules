...
class BaseNavAgent:
    ...
    def __init__(
        ...
        logger.warning("[SYSTEM_PROMPT_DEBUG] agent=%s user_ltm=%r system_message_tail=%r",
            self.agent_name, user_ltm, system_message[-200:])
        ...
        for tool_path in additional_tool_paths:
            ...
            if os.path.isdir(tool_path):
                ...
                try:
                    ...
                    for filename in os.listdir(tool_path):
                        if filename.endswith(".py"):
                            ...
                            add_event(EventType.TOOL, EventData(detail=f"Registering tool: {filename}"),)
                finally:
                ...
                try:                    
                    ...
                    add_event(EventType.TOOL, EventData(detail=f"Registering tool: {module_name}"),)
                finally:
                    sys.path.remove(parent_dir)
            else:
