...
class SimpleHercules:
    ...
    def _log_model_call(self, agent_name: str, messages: list[AnyMessage]) -> None:
        serialized = [
            {"role": getattr(message, "type", message.__class__.__name__), "content": getattr(message, "content", "")}
    ...
    async def _executor_node(self, state: AgentState) -> dict[str, Any]:
        ...
        if not next_step or target_helper == "not_applicable":
            ...
            return {"step_token_log": state.get("step_token_log", []) + [executor_entry],
            "step_timings": state.get("step_timings", []) + [{
                "node": "executor",
                "turn": len([
                    t for t in state.get("step_timings", [])
                    if t["node"] == "executor"
                ]) + 1,
                "duration": elapsed,
            }],
    ...
    def _assertion_node(self, state: AgentState) -> dict[str, Any]:
        ...
        final_response = str(state.get("final_response") or (
            "The test passed." if is_passed else "The test failed."
        ))
