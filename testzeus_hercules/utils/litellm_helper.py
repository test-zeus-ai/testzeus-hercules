...
from testzeus_hercules.utils.llm_helper import convert_model_config_to_langchain_format, create_chat_model
...
def ensure_litellm_provider() -> None:
    ...
    if not config_file:
        raise ValueError(
            "LiteLLM is required. Set AGENTS_LLM_CONFIG_FILE to agents_llm_config.json "
            "with a 'litellm' provider section and AGENTS_LLM_CONFIG_FILE_REF_KEY=litellm."
        )
    ...
    if active != LITELLM_PROVIDER:
        raise ValueError(
            f"Only the '{LITELLM_PROVIDER}' provider is supported; "
            f"active provider is '{active}'. "
            f"Ensure agents_llm_config.json contains a '{LITELLM_PROVIDER}' section."
        )
