...
::-webkit-scrollbar-thumb:hover {
  ...
  #hercules-closebutton:hover{
    transform: scale(1.1);
  }

  #hercules-closebutton:active{
    transform: scale(0.8);
  }

  @keyframes hercules-gradient-animation {
  0% {background-position: 100% 0%}
  100% {background-position: 15% 100%}
  }

  @keyframes hercules-rotate {
    100% {
  ...
  .hercules-ui-automation-highlight {
  ...
  animation: hercules-rotate 1s linear infinite;
  }
  ...
  .hercules-processingLine {
    ...
    animation: hercules-gradient-animation 3s linear infinite;
  }

  .hercules-initStateLine{
  background: lightgray;
  }

  .hercules-doneStateLine{
  background: lightgreen;
  }
  ...
  #hercules-send-btn:hover{
  transform: scale(1.1);
  }

  .hercules-highlight_overlay{
...
.hercules-toggle:focus {
  border: none; !important;
  outline: none; !important;
}
.hercules-toggle:before {
  content: "";
  display: block;
  position: absolute;
  z-index: 2;
...
.hercules-toggle:checked {
  background-color: #786E96;
}

.hercules-toggle:checked:before {
  left: 20px;
}
`;
...
function showCollapsedOverlay(processing_state = "processing", steps) {
  ...
  let collapsed_hercules = document.createElement("div");
  collapsed_hercules.id = "hercules-overlay";
  collapsed_hercules.classList.add("hercules-collapsed");
  collapsed_hercules.style.backgroundColor = "transparent";
  collapsed_hercules.setAttribute("aria-hidden", "true");
  collapsed_hercules.style.justifyContent = "center";
  ...
  borderdiv.id="herculesOverlayBorder";
  logodiv.style.position = "absolute";
  logodiv.style.justifySelf = "center";
  wrapper.appendChild(borderdiv);
  wrapper.appendChild(logodiv);
  collapsed_hercules.appendChild(wrapper);
  document.body.appendChild(collapsed_hercules);
  
  updateOverlayState(processing_state, true);
  
  let element = document.getElementById('hercules-overlay');
  document.getElementById('hercules-overlay').addEventListener('mouseover', function () {
    this.style.transform = 'scale(1.1)';
  });

  document.getElementById('hercules-overlay').addEventListener('mouseout', function () {
    this.style.transform = 'scale(1)';
  });
  document.getElementById('hercules-overlay').addEventListener('click', function () {
    let ui_state = document.getElementById("herculesOverlayBorder").classList.contains("hercules-init") ? "init" : document.getElementById("herculesOverlayBorder").classList.contains("hercules-processing") ? "processing" : "done";
...
function clearOverlayMessages(keep_default=false) {
  try {
    let chatBox = document.getElementById('hercules-chat-box');
    if (!chatBox) {
      return;
    }
...
{
  if (is_collapsed) {
    let borderdiv = document.getElementById("herculesOverlayBorder");
    if (processing_state === "init"){
      borderdiv.classList.add("hercules-init");
      borderdiv.classList.remove("hercules-processing");
      borderdiv.classList.remove("hercules-done");
    }
    else if (processing_state === "processing"){
      borderdiv.classList.remove("hercules-init");
      borderdiv.classList.add("hercules-processing");
      borderdiv.classList.remove("hercules-done");
    }
    else if (processing_state === "done"){
      borderdiv.classList.remove("hercules-init");
      borderdiv.classList.remove("hercules-processing");
      borderdiv.classList.add("hercules-done");
    }
  } else {
    let animation = document.getElementById("herculesExpandedAnimation");
    if (processing_state === "init"){
      animation.classList.remove("hercules-processingLine");
      animation.classList.add("hercules-initStateLine");
      animation.classList.remove("hercules-doneStateLine");
      enableOverlay();
    }
    else if (processing_state === "processing"){
      animation.classList.add("hercules-processingLine");
      animation.classList.remove("hercules-initStateLine");
      animation.classList.remove("hercules-doneStateLine");
      disableOverlay();
    }
    else if (processing_state === "done"){
      animation.classList.remove("hercules-processingLine");
      animation.classList.remove("hercules-initStateLine");
      animation.classList.add("hercules-doneStateLine");
      enableOverlay();
    }
  }
}
...
function showExpandedOverlay(processing_state = "init", show_steps=true) {
  ...
  // let hercules_logo = `<svg width="85" height="12" viewBox="0 0 85 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0 11.8027L3.43562 0.213699H8.35069L11.8027 11.8027H9.3863L8.23562 7.85753H3.53425L2.38356 11.8027H0ZM4.10959 5.86849H7.66027L6.18082 0.80548H5.58904L4.10959 5.86849Z" fill="#6B6673"/><path d="M19.0946 12C15.6096 12 13.7028 9.56712 13.7028 6.09863C13.7028 2.4 15.9055 0 19.4562 0C22.4151 0 24.5685 1.70959 24.9631 4.35616H22.6124C22.3822 2.87671 21.2151 1.9726 19.5713 1.9726C17.3192 1.9726 16.0535 3.58356 16.0535 6.09863C16.0535 8.35068 17.0726 10.011 19.637 10.011C21.7576 10.011 22.974 8.94247 22.974 7.15068H19.374V5.40822H23.9768C24.8151 5.40822 25.2918 5.85205 25.2918 6.69041V11.8027H23.0069V10.7671L23.4672 8.92603H22.8589C22.8754 9.6 22.4973 12 19.0946 12Z" fill="#6B6673"/><path d="M28.7192 11.8027V0.213699H37.3987V2.20274H31.0206V5.04658H36.5768V6.95342H31.0206V9.8137H37.3987V11.8027H28.7192Z" fill="#6B6673"/><path d="M40.533 11.8027V0.213699H45.0536L49.1631 11.211H49.7385L49.3275 9.76438V0.213699H51.6125V11.8027H47.0919L42.9823 0.80548H42.3905L42.8179 2.25205V11.8027H40.533Z" fill="#6B6673"/><path d="M54.4378 0.213699H64.4159V2.20274H60.5693V11.8027H58.2844V2.20274H54.4378V0.213699Z" fill="#6B6673"/><path d="M63.9401 6.6411H72.4551V8.30137H63.9401V6.6411Z" fill="#6B6673"/><path d="M75.3643 11.8027V0.213699H84.0438V2.20274H77.6657V5.04658H83.2219V6.95342H77.6657V9.8137H84.0438V11.8027H75.3643Z" fill="#6B6673"/></svg>`;
  let hercules_logo = `<svg id="eVUlKYcEWhD1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 85 12" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" project-id="473afff08bed4a3187c0146f5aa0a139" export-id="38a7b089750b43b3b8c780da28a4cd85" cached="false"><text dx="0" dy="0" font-family="&quot;eVUlKYcEWhD1:::Open Sans&quot;" font-size="15" font-weight="600" transform="translate(23.5 11.263314)" fill="#6b6673" stroke-width="0"><tspan y="0" font-weight="600" stroke-width="0"><![CDATA[Hercules]]></tspan></text><style><![CDATA[@font-face {font-family: 'eVUlKYcEWhD1:::Open Sans';font-style: normal;font-weight: 600;font-stretch: normal;src: url(data:font/ttf;charset=utf-8;base64,) format('truetype');}}]]></style></svg>`;
  ...
  newDiv.id = "hercules-overlay";
  newDiv.classList.add("hercules-highlight_overlay");
  ...
  logoDiv.style.backgroundImage = `url('data:image/svg+xml;utf8,${encodeURIComponent(hercules_logo)}')`;
  logoDiv.style.backgroundRepeat = "no-repeat";
  logoDiv.style.backgroundSize = "contain";
  ...
  closeButton.id = "hercules-closebutton";
  ...
  closeButton.onclick = function () {
    let ui_state = document.getElementById("herculesExpandedAnimation").classList.contains("hercules-initStateLine") ? "init" : document.getElementById("herculesExpandedAnimation").classList.contains("hercules-processingLine") ? "processing" : "done";
  ...
  animation.id = "herculesExpandedAnimation";
  ...
  chatContainer.className = "hercules-chat-container";

  let chatBox = document.createElement("div");
  chatBox.id = "hercules-chat-box";

  let chatInput = document.createElement("div");
  chatInput.className = "hercules-chat-input";
  chatBox.appendChild(chatInput);

  let inputContainer = document.createElement("div");
  inputContainer.className = "hercules-input-container";
  inputContainer.id = "hercules-input-container";
  let userInput = document.createElement("textarea");
  userInput.id = "hercules-user-input";
  ...
  userInput.addEventListener('input', function(event) {
    ...
    if (text.trim() == "") {
      ...
      let sendBtn = document.getElementById('hercules-send-btn');
  ...
  toggleSwitch.className = "hercules-toggle";
  ...
  disclaimer.textContent = "hercules may make mistakes. Verify key info.";

  newDiv.appendChild(disclaimer);

  document.body.appendChild(newDiv);
  updateOverlayState(processing_state, false);
  document.getElementById('hercules-send-btn').addEventListener('click', function () {
    let task = document.getElementById('hercules-user-input').value
    let task_trimmed = task.trim();
    if (task_trimmed && !isDisabled() && task_trimmed.length > 0) {
      if (awaitingUserResponse) {
        addUserMessage(task);
        document.getElementById('hercules-user-input').value = "";
      } else {
  ...
  userInput.addEventListener('focus', function() {
    if (window.getSelection().rangeCount > 0) {
        ...
        if (selectedText) {
          document.getElementById('hercules-user-input').value = selectedText +  '\n';
          setTimeout(function() {
...
userInput.addEventListener('blur', function() {
  ...
  document.getElementById('hercules-user-input').addEventListener('keydown', function (event) {
    // Check if the pressed key is the Enter key
    if (event.key === "Enter") {
      event.preventDefault();

      let targetElement = document.getElementById('hercules-send-btn');
...
function focusOnOverlayInput() {
  document.getElementById('hercules-user-input').focus();
}

function addMessage(message, sender, message_type = "plan") {
  let newDiv = document.createElement("div");
  newDiv.classList.add("hercules-chat-input");
  let chatDiv = document.createElement("div");
  chatDiv.classList.add("hercules-chat");
  ...
  if (sender === "system") {
    newDiv.classList.add("hercules-agent");
    chatDiv.classList.add("hercules-systemMessage", "hercules-pre-line");
      if (message_type === "step") {
      chatDiv.classList.add("hercules-agentstep");
      }
      else if (message_type === "plan" || message_type === "question") {
        chatDiv.classList.add("hercules-agentplan");
      }

      else if (message_type === "answer") {
      chatDiv.classList.add("hercules-agentanswer");
      }
    ...
    newDiv.classList.add("hercules-user")
    chatDiv.classList.add("hercules-usertext", "hercules-pre-line");
    chatDiv.textContent = parsedMessage;
  }
...
function disableOverlay() {
  let input_field= document.getElementById("hercules-user-input");
  if(input_field){
